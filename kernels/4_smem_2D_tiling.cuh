...
__global__ void smem_2D_tiling(int M, int N, int K, T alpha, T *A, T *B, T beta, T *C){
  ...
  for(int bk = 0; bk < K; bk += TILE_K){
    ...
    for(int offset = 0; offset < TILE_M; offset += strideA){
        As[(offset + threadIdx.x / TILE_K) * TILE_K + threadIdx.x % TILE_K] = A[(offset + threadIdx.x / TILE_K) * K + threadIdx.x % TILE_K];
    }
    ...
    for(int k = 0; k < TILE_K; k++){
        for(int i = 0; i < THREAD_M; i++){
            reg_A[i] = As[(thread_row * THREAD_M + i) * TILE_K + k];
        }
